...
export function Tourbottom({ settourbottom }) {
  var bottomColor = [];
  var bottomRangeDark = JSON.parse(sessionStorage.getItem("value")).bottom[
    "dark"
  ];
  var bottomRangeLight = JSON.parse(sessionStorage.getItem("value")).bottom[
    "light"
  ];
  ...
  return (
    <div className="palettet">
      {bottomColor.map((el) => (
        <Elementsnbottom el={el} settourbottom={settourbottom} />
      ))}
    </div>
  );
}
